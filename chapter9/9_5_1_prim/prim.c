...
void init(struct graph *g)
{
    ...
    list_insert(&g->vertex[4].l, 1, 1);
    list_insert(&g->vertex[4].l, 2, 3);
    list_insert(&g->vertex[4].l, 5, 7);
    ...
    list_insert(&g->vertex[5].l, 2, 10);
    list_insert(&g->vertex[5].l, 4, 7);

    g->vertex[6].v = 6;
    g->vertex[6].known = 0;
    g->vertex[6].dis = INT_MAX;
    g->vertex[6].path = 0;
    g->vertex[6].l.element = 3;
    g->vertex[6].l.val = 5;
    g->vertex[6].l.next = NULL;
    list_insert(&g->vertex[6].l, 4, 8);
    list_insert(&g->vertex[6].l, 7, 1);
    ...
    list_insert(&g->vertex[7].l, 4, 4);
    list_insert(&g->vertex[7].l, 5, 6);
...
void print_prim_result(struct graph *g)
{
    int i = 0;
    ...
    printf("\npath:\t");
    for(i=1; i<=g->capacity; i++) {
        printf("%d\t", g->vertex[i].path);
    }
    printf("\ndis:\t");
    for(i=1; i<=g->capacity; i++) {
        printf("%d\t", g->vertex[i].dis);
    }
}
    ...
    while (1){
        ...
        while(tmp) {
            if(g->vertex[tmp->element].known == 0) {
                    if(tmp->val < g->vertex[tmp->element].dis) {
                        g->vertex[tmp->element].dis = tmp->val;
